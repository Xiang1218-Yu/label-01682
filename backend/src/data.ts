/**
 * 后端航班数据 & 实时查询逻辑。
 * 作为服务端数据源的单一事实来源，价格与余票基于实时时间动态计算。
 */

// ─── 类型（后端独立定义，不依赖前端 src/types） ───
...
export const popularCities: CityAirport[] = [
...
export function searchCities(keyword: string): CityAirport[] {
...
export const allFlights: FlightOffer[] = [
  {
    id: 'FL001', segments: [{
...
function realtimePricing(flight: FlightOffer, date: string, now: Date): FlightOffer {
  const d = new Date(date)
  if (isNaN(d.getTime())) return flight
  const minuteOfDay = now.getHours() * 60 + now.getMinutes()
  const secondFactor = now.getSeconds() / 60
  ...
  const daysAhead = Math.max(0, Math.floor((d.getTime() - now.getTime()) / 86400000))
  const urgencyFactor = daysAhead <= 1 ? 1.25 : daysAhead <= 3 ? 1.15 : daysAhead <= 7 ? 1.05 : 1.0
  const peakHourFactor = (minuteOfDay >= 540 && minuteOfDay <= 720) || (minuteOfDay >= 840 && minuteOfDay <= 1080) ? 1.03 : 0.98
  const flightSeed = parseInt(flight.id.replace('FL', ''), 10) || 0
  const seed = (dayHash + flightSeed * 17 + Math.floor(minuteOfDay / 15)) % 20
  const baseFluct = 0.92 + seed * 0.01 + secondFactor * 0.005
  const priceFactor = weekendFactor * urgencyFactor * peakHourFactor * baseFluct
  const newPrice = Math.round(flight.price * priceFactor)
  const seatSeed = (dayHash + flightSeed * 3 + Math.floor(minuteOfDay / 30)) % 7
  const seatDelta = seatSeed - 3
  const newSeats = Math.max(1, Math.min(30, flight.seatsLeft + seatDelta))
  const tags = [...(flight.tags || [])]
  if (daysAhead <= 3 && !tags.includes('即将出发')) tags.push('即将出发')
  if (newSeats <= 3 && !tags.includes('余票紧张')) tags.push('余票紧张')
  return { ...flight, price: newPrice, totalPrice: newPrice + flight.tax, seatsLeft: newSeats, tags }
}

// ─── 查询逻辑 ───

export function searchFlights(from: string, to: string, cabinClass?: string, date?: string): FlightOffer[] {
  const now = new Date()
  if (!from && !to && !cabinClass) {
    return date ? allFlights.map((f) => realtimePricing(f, date, now)) : allFlights
  }
  ...
  return filtered.map((f) => realtimePricing(f, date, now))
}
...
export function generatePriceCalendar(currentDate: string, from?: string, to?: string, cabinClass?: string): PriceCalendarItem[] {
  ...
  const matchingBase = allFlights.filter((f) => {
    const departCode = f.segments[0].departAirport.code
    const arriveCode = f.segments[f.segments.length - 1].arriveAirport.code
    const fromMatch = !from || departCode === from || f.segments.some((s) => s.departAirport.code === from)
    const toMatch = !to || arriveCode === to || f.segments.some((s) => s.arriveAirport.code === to)
  ...
  for (let i = -3; i <= 10; i++) {
    ...
    if (matchingBase.length > 0) {
      const priced = matchingBase.map((f) => realtimePricing(f, dateStr, now))
      dayPrices.push(Math.min(...priced.map((f) => f.price)))
    } else {
      dayPrices.push(2980)
    }
  }
