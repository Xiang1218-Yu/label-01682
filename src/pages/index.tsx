...
function IndexPage({ onNavigateToOrder }: { onNavigateToOrder: (flightId: string) => void }) {
  ...
  const filteredFlights = useMemo(() => {
    ...
    if (searchParams.filters?.priceRange) {
      const [minPrice, maxPrice] = searchParams.filters.priceRange
      result = result.filter(f => f.price >= minPrice && f.price <= maxPrice)
    }
    
    // 多条件筛选 - 出发时间段
    if (searchParams.filters?.departTimeRange) {
      const [startHour, endHour] = searchParams.filters.departTimeRange
      result = result.filter(f => {
        // 处理可能包含日期的时间格式，如 "2023-10-01 08:30" 或 "08:30"
        const timePart = f.segments[0].departTime.split(' ').pop() || ''
        const departHour = parseInt(timePart.split(':')[0])
        return !isNaN(departHour) && departHour >= startHour && departHour < endHour
      })
    }
    
    // 多条件筛选 - 到达时间段
    if (searchParams.filters?.arriveTimeRange) {
      const [startHour, endHour] = searchParams.filters.arriveTimeRange
      result = result.filter(f => {
        const lastSegment = f.segments[f.segments.length - 1]
        // 处理可能包含日期的时间格式
        const timePart = lastSegment.arriveTime.split(' ').pop() || ''
        const arriveHour = parseInt(timePart.split(':')[0])
        return !isNaN(arriveHour) && arriveHour >= startHour && arriveHour < endHour
      })
    }
    ...
    result.sort((a, b) => {
      ...
      if (sort.field === 'price') {
        cmp = a.price - b.price
      } else if (sort.field === 'departTime') {
        cmp = a.segments[0].departTime.localeCompare(b.segments[0].departTime)
      } else if (sort.field === 'arriveTime') {
        cmp = a.segments[a.segments.length - 1].arriveTime.localeCompare(b.segments[b.segments.length - 1].arriveTime)
      } else if (sort.field === 'duration') {
        cmp = getTotalDurationMinutes(a.segments) - getTotalDurationMinutes(b.segments)
      } else if (sort.field === 'airline') {
        // 按航空公司名称拼音排序
        cmp = a.segments[0].airline.localeCompare(b.segments[0].airline, 'zh-CN')
      }
